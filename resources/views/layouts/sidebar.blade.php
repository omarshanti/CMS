<div class="col-md-4 col-xl-3">
    <div class="sidebar px-4 py-md-0">
      ...
      <form class="input-group" method="{{route('home')}}" method="GET">
      ...
      <div class="row link-color-default fs-14 lh-24">
        @foreach ($catnum as $cat)
        <div class="col-6"><a href="{{route('posts.category',$cat->id)}}">{{$cat->name}}</a></div>
       @endforeach
      ...
      <div class="gap-multiline-items-1">
        @foreach($tagnum as $tag)
          
 
        <a class="badge badge-secondary" href="{{route('posts.tag',$tag->id)}}">{{$tag->tag}}</a>
        @endforeach
      </div>
